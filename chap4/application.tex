...
The the smartphone application a loading spinner pops up when information is being downloaded, as seen in Figure~\ref{downloadSpinner}. However, such a spinner was not implemented in the Google Glass application. The reason for not implementing such a download spinner in the Google Glass application was because the AsyncTask, in order to display anything during the execution of the AsyncTask, requires access to the active activity TODO explain why no spinner in glass application

	\begin{figure}[ht!]
		\centering
		\includegraphics[width=70mm]{images/demo/smartphone/loadingSpinner}
		\caption{The loading spinner in the smartphone application.}
		\label{downloadSpinner}
	\end{figure}
